...
import { updateSettings, setShowSettings } from './features/writeGameSlice';

type CardKey = 'simplified' | 'traditional' | 'pinyin' | 'english' | 'structure';

const WriteSettings: React.FC = () => {
  const dispatch = useDispatch();
  const { settings, showSettings } = useSelector((state: RootState) => state.write);
  ...
  const renderDropdownItem = (item: CardKey, type: 'front' | 'back') => {
    const isSelected = (type === 'front' ? front : back) === item;
    return (
      <TouchableOpacity
    ...
    <Modal visible={showSettings} animationType="slide" transparent={true}>
      <View style={styles.modalContainer}>
        <View style={styles.modalContent}>
          <Text style={styles.title}>Write Game Settings</Text>
          <View style={styles.buttonContainer}>
            <Text style={styles.label}>Front:</Text>
            <TouchableOpacity style={styles.dropdownButton} onPress={() => setDropdownVisible('front')}>
              <Text style={styles.dropdownText}>{front}</Text>
            </TouchableOpacity>
          </View>
          <View style={styles.buttonContainer}>
